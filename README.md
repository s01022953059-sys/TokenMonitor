...
四源数据合并后做**跨源去重**：同一时间窗口（2 秒）内 + 同模型 + 同 Token 量视为同一事件，只计一次。避免同一笔请求被多个数据源重复计入。
...
- 为每台设备生成 `User_XXXXX` 匿名 ID，只提交今日 Token 数量和工具汇总
...
- 公开社区数据无需凭据即可查看；当前版本提交匿名统计仍要求本机已配置 GitCode 凭据
...
| `GET /api/community/optin?enabled=true/false` | 开启或关闭社区统计上报 |
...
| **无控制台窗口** | v1.3.92+ Win 端用 `TokenMonitorLauncher.exe` 弹独立 WebView2 窗口 (隐藏 cmd 窗口), `TokenMonitor.exe` 仍保留 console 模式 |
...
| **社区数据上报** | 公开排行可直接查看；提交匿名统计需要本机安装 Git 并已配置 GitCode 凭据 |
...
curl -L -o "Token Monitor.dmg" \
  "https://api.gitcode.com/baggiopeng/TokenMonitor/releases/download/v1.4.18/Token%20Monitor.dmg"
...
curl -L -o TokenMonitor-win.zip \
  "https://api.gitcode.com/baggiopeng/TokenMonitor/releases/download/v1.4.18/TokenMonitor-win.zip"
```
...
├── scanner.py                 # macOS 数据采集 (四源 + 去重 + 归一化)
...
两版的数据采集逻辑完全对齐（四源 + 去重 + 归一化 + DeepSeek 余额），API 返回格式一致，前端同一份 `index.html`。
...
最新版本：[v1.4.18](https://gitcode.com/baggiopeng/TokenMonitor/releases/v1.4.18)

- macOS: [Token Monitor.dmg](https://api.gitcode.com/baggiopeng/TokenMonitor/releases/download/v1.4.18/Token%20Monitor.dmg)
- Windows: [TokenMonitor-win.zip](https://api.gitcode.com/baggiopeng/TokenMonitor/releases/download/v1.4.18/TokenMonitor-win.zip)
