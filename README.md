...
| **无控制台窗口** | v1.3.88+ Win 端用 `TokenMonitorLauncher.exe` 弹独立 WebView2 窗口 (隐藏 cmd 窗口), `TokenMonitor.exe` 仍保留 console 模式 |
...
后续升级 (v1.3.51 → v1.3.88) 因 app 内有自更新, 装新版本后**同样需要重新做一次右键"打开"** (Gatekeeper 重新检查).
如果 v1.3.88 之前装过老版本 (.accessory 模式, dock 没图标), lsregister 重索引 + killall Dock 即可让 dock 重新出现 T+火焰 图标.
...
curl -L -o "Token Monitor.dmg" \
  "https://api.gitcode.com/baggiopeng/TokenMonitor/releases/download/v1.3.88/Token Monitor.dmg"
...
curl -L -o TokenMonitor-win.zip \
  "https://api.gitcode.com/baggiopeng/TokenMonitor/releases/download/v1.3.88/TokenMonitor-win.zip"
```
...
最新版本：[v1.3.88](https://gitcode.com/baggiopeng/TokenMonitor/releases/v1.3.88)

- macOS: [Token Monitor.dmg](https://api.gitcode.com/baggiopeng/TokenMonitor/releases/download/v1.3.88/Token%20Monitor.dmg)
- Windows: [TokenMonitor-win.zip](https://api.gitcode.com/baggiopeng/TokenMonitor/releases/download/v1.3.88/TokenMonitor-win.zip)
