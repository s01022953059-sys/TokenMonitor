...
- **macOS 昵称写入鉴权**: WKWebView 从 `file://` 请求本地 API 时发送 `Origin: null`。Swift 每次启动生成临时凭据并同时注入 WebView 与 Python 子进程；只有携带匹配 `X-Token-Monitor-Client` 的 `null` 来源可写昵称，远程来源始终拒绝
