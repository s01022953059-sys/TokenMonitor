...
20. **发现疑似 Bug 必须先增测试用例再修代码**：鹏帅报告任何数据异常（用量不对、去重怀疑、社区不更新等），必须先写能复现该问题的回归测试（放在 `tests/test_*.py`），确认测试失败，再定位根因并修复，最后确认测试通过。即使最终确认不是代码 Bug（如旧实例缓存、时区误解），也要保留测试用例防止未来回归。测试覆盖维度至少包括：数据源口径（时间戳单位、缓存分离）、跨源去重（不误合并不漏算）、社区上报（by_tool 动态含新工具、today_tokens 含新工具用量）。
...
### v1.4.41 (2026-07-17)
- 调用详情筛选在完整日快照上完成，支持工具、模型和起止时间组合过滤。
- 筛选后的列表、统计摘要和分页必须使用同一过滤结果；macOS 与 Windows API 保持一致。

### v1.4.40 (2026-07-15)
- 首页左侧工具圆环的辅助指标为去重后的调用次数，右侧模型圆环的辅助指标为缓存命中率。
- 缓存命中率统一按 `input_cached / input_tokens` 计算；macOS 与 Windows 共用同一份前端实现。
...
## 社区排名归档日期口径（2026-07-29）

- VPS 每日归档固定按 `Asia/Shanghai` 日期在北京时间 23:55 执行，不能使用 UTC 23:55，否则客户端的 `report_date` 已进入次日，会把前一天错误归档为空。
- 排名趋势右侧姓名必须统一取最近一个至少存在一条 1–10 名有效排名的归档日；不得让每条折线分别寻找自己的最后数据点。状态栏需明确显示右侧排名对应日期。
- 排名趋势同步展示当前所选范围的累计总榜，按累计 Token 排序，并显示每人的累计用量与有记录天数；宽屏固定在图表右侧，窄屏移到图表下方，颜色与折线保持一致。
- 排名趋势对所选周期累计 TOP10 的每个归档日都生成有效排名点；当天未使用按 0 Token 记录，多个 0 按该日期之前的累计 Token、历史活跃天数和稳定身份顺序排序，前端不得再把 0 转为 `null` 造成断线。
- 排名趋势的折线标签和区间总榜不得直接信任历史归档里的旧 `display_name`；社区聚合 API 返回所有当前公开昵称的 ID 映射，前端按不可变匿名 ID 覆盖显示名，使改名刷新后立即同步且不改写历史 Token/排名。
- 升级涉及空归档修复时需更新前端排名缓存版本，避免客户端继续命中旧的 5 分钟空结果；历史快照只能依据可核验的当天报告恢复，不能推测补齐。
