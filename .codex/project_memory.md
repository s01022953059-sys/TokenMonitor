...
- **统计页面缓存**: 趋势图和热力图使用按自然日及查询范围隔离的 `localStorage` 缓存，打开先展示缓存、后台刷新；启动后预取历史趋势与近 30 天热力图。热力图默认变量必须与 UI 默认 Tab 的 30 天一致，不能表面选中 30 天却扫描 365 天
- **社区参与人数口径**: 顶部社区规模卡片展示“历史参与用户”并使用 `all_reporters`；今日排行榜和“我的今日排名”仍使用当日有效上报人数作为分母，不能混用两个口径
