...
# 第二次打开应直接命中缓存，密集末端姓名经过布局后必须保持可读间距。
RANK_TREND=$($PWCLI eval "async () => { const originalFetch = window.fetch; const dates = ['2026-07-21','2026-07-22','2026-07-23']; const series = Array.from({length:10}, (_, index) => ({id:'User_' + index,display_name:'用户' + index,ranks:[index + 1,((index + 1) % 10) + 1,index + 1],tokens:[100,200,300]})); let fetchCalls = 0; _rankHistory.cache.clear(); localStorage.removeItem(RANK_HISTORY_CACHE_PREFIX + '30'); window.fetch = async (url, options) => { if (String(url).includes('/api/community/history')) { fetchCalls += 1; return new Response(JSON.stringify({dates,series,participant_count:12,participant_count_complete:true,data_status:'ok'}), {status:200,headers:{'Content-Type':'application/json'}}); } return originalFetch(url, options); }; document.getElementById('communityModal').classList.remove('active'); document.getElementById('rankHistoryModal').classList.add('active'); await loadRankHistory(30, {forceRefresh:true}); await new Promise(resolve => setTimeout(resolve, 950)); const chart = _rankHistory.chart; const canvas = document.getElementById('rankHistoryChart'); const pixels = canvas.getContext('2d').getImageData(0, 0, canvas.width, canvas.height).data; const nonBlank = pixels.some((value, index) => index % 4 === 3 && value > 0); destroyRankHistoryChart(); const started = performance.now(); await loadRankHistory(30); const cachedFast = performance.now() - started < 100 && fetchCalls === 1 && Boolean(_rankHistory.chart); const laidOut = layoutRankHistoryEndLabels([{y:100},{y:100},{y:102}], 80, 160, 16); const readableGap = laidOut.every((item, index) => index === 0 || item.labelY - laidOut[index - 1].labelY >= 15.9); const result = [chart && chart.config.type === 'line',chart && chart.data.datasets.length === 10,document.getElementById('rankHistoryStatus').innerText.includes('统计 12 位参与者'),nonBlank,cachedFast,readableGap].join('|'); window.fetch = originalFetch; return result; }")
printf '%s\n' "$RANK_TREND" | grep -q 'true|true|true|true|true|true'
