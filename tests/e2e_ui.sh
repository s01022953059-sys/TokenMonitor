...
RANK_TREND=$($PWCLI eval "async () => { const originalFetch = window.fetch; const dates = ['2026-07-21','2026-07-22','2026-07-23']; const series = Array.from({length:10}, (_, index) => ({id:'User_' + index,display_name:'用户' + index,ranks:[index + 1,((index + 1) % 10) + 1,index + 1],tokens:[100,200,300]})); window.fetch = async (url, options) => String(url).includes('/api/community/history') ? new Response(JSON.stringify({dates,series,participant_count:12,participant_count_complete:true,data_status:'ok'}), {status:200,headers:{'Content-Type':'application/json'}}) : originalFetch(url, options); document.getElementById('communityModal').classList.remove('active'); document.getElementById('rankHistoryModal').classList.add('active'); await loadRankHistory(30); await new Promise(resolve => setTimeout(resolve, 950)); const chart = _rankHistory.chart; const canvas = document.getElementById('rankHistoryChart'); const pixels = canvas.getContext('2d').getImageData(0, 0, canvas.width, canvas.height).data; const nonBlank = pixels.some((value, index) => index % 4 === 3 && value > 0); const result = [chart && chart.config.type === 'line',chart && chart.data.datasets.length === 10,document.getElementById('rankHistoryStatus').innerText.includes('统计 12 位参与者'),nonBlank].join('|'); window.fetch = originalFetch; return result; }")
printf '%s\n' "$RANK_TREND" | grep -q 'true|true|true|true'
