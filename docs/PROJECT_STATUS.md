...
- 新增社区用量排行: 匿名 ID、今日聚合、历史参与用户、完整个人排名、Top 10、工具占比；上报在后台自动完成，页面不暴露同步按钮、时间或传输状态
- 社区页增加紧凑动态栏，根据已有匿名聚合结果轮播榜首、参与人数、今日总量和热门工具，不新增上报字段
- 社区 Dashboard 使用按自然日隔离的本地聚合缓存并启动预取，打开时先显示缓存、后台更新，避免冷加载长期停留
- 趋势图与热力图采用同样的 stale-while-revalidate 缓存；默认热力图范围修正为近 30 天，并在启动后静默预取
- Windows 托盘双击直接打开首页，右键保留菜单；热力图月份轴和日期网格统一尺寸，避免 WebView2 布局错位
- 新社区用户首次打开排行时会立即后台登记并刷新排名，不再只显示本地匿名 ID
...
- 新安装和旧版 `optin=false` 用户均自动加入社区统计，启动后约 5 秒首次上报；不再提供需要用户确认的加入步骤
- cc-switch `_codex_session` 使用事件自身模型，不再用当前活动 provider 覆盖历史记录，避免 GPT-5.6 被归入 GPT-5.5
...
- v1.4.22 发布社区昵称，当前规则为：昵称与匿名 ID 分离、SQLite 原子重名检查、滚动 24 小时最多修改 3 次、30 天旧名保护及风险名称防护
...
| `build_windows.sh` | Go 交叉编译主程序并嵌入 `TokenMonitor-Setup.exe`；复制同内容的 `TokenMonitor.exe` 作为旧版在线更新迁移入口 |
| `release_all.sh` | **一键发布**: 清空 build/ → Mac DMG + Windows Setup EXE → 上传 GitCode Release → 清空 build/ |
...
# 3. 一键发布 (Mac DMG + Windows 安装程序同时构建上传)
bash release_all.sh
```
...
3. `build_windows.sh` Go 交叉编译用户级安装程序 → 上传 `TokenMonitor-Setup.exe`，并上传同内容的 `TokenMonitor.exe` 兼容旧客户端在线迁移
4. 清空 `build/` 目录
5. 验证 Release 附件列表
...
- Mac: `https://gitcode.com/baggiopeng/TokenMonitor/releases/download/v1.4.31/Token Monitor.dmg`
- Windows: `https://gitcode.com/baggiopeng/TokenMonitor/releases/download/v1.4.31/TokenMonitor-Setup.exe`
